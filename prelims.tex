...
\[
1-\zeta = 2^{-(\tilde{m}-1)+(n-\log{|B_t|}+\log{1/\epsilon})}.
\]
...
\begin{proof}
The proof of Lemma~\ref{lem:distributional advise suffices ss} follows the structure of the proof of Lemma~\ref{lem:distributional advise suffices} with the following inequality for smooth min-entropy.

\begin{align*}
\expe_{Z} [\Haveps(W_Z| ss)] &= -\log{\Pr[Z\in Z']\expe_{Z\in Z'} 2^{\Haveps(W_Z| ss)}+ \Pr[Z\not\in Z']\expe_{Z\not \in Z'} 2^{-\Haveps(W_Z| ss)]}}\\
&\le -\log{ 1* 2^{-(\tilde{m}-1)} + (1-\zeta)*2^{-(n-\log{|B_t|}+\log{1/\epsilon})} } \\
&\le -\log{ 1* 2^{-(\tilde{m}-1)} + 2^{-(\tilde{m}-1)+(n-\log{|B_t|}+\log{1/\epsilon})-(n-\log{|B_t|}+\log{1/\epsilon})} }\\
&\le -\log{ 2^{-(\tilde{m}-1)}+ 2^{-(\tilde{m}-1)}} = \tilde{m}.
\end{align*}
\end{proof}
