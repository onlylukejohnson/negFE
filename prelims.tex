...
\]
For a metric space $(\mathcal{M}, \dis)$ let $B_t(x) = \{y | \dis(x, y)\le t\}$. If the size of $B_t(x)$ is the same for all points $x$ we use $|B_t|$ to denote this quantity and say that the \emph{size of balls is center independent}.  This is true for the Hamming metric.  All logarithms are base $2$. For a set $X$ let $U_X$ denote the uniform distribution over that set.  For a distribution $W$ let $\supp(W)$ denote the support of the distribution.
...
there is some $z'\in \mathcal{Z}'$ for which no  $(\zo^n, \min\{\tilde{m}, \zeta\}, t, \epsilon, \delta, \ell)$-secure sketch $(\sketch_{z'}, \rec_{z'})$  exists.
