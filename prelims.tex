...
\begin{lemma}
    ...
    Let $\vec{X} = (X_1, X_2, \ldots, X_k)$ be random variables and let $Y$ be a random variable arbitrarility correlated with $\vec{X}$. Suppose that $\expe_i[\Hoo(X_i | Y) \ge \Delta$. 
    Let $\alpha > 0$, then \[\Pr_i[ \Hoo(X_i|Y)< \Delta - \log{\alpha}]\le 1/\alpha.\] 
\end{lemma}
Let $\expe_i [\Hav(X_i|Y)] = \Delta$. Then by Markov's inequality
\begin{align}
    \Problim{i}{\max\limits_{x} \Prob{X_i = x \,|\, Y = y} \geq \alpha \cdot 2^{-\Delta}} &\leq \frac{1}{\alpha} \\
    \Problim{i}{\log{\max\limits_{x} \Prob{X_i = x \,|\, Y = y}} \geq \log{\alpha} -\Delta} &\leq \frac{1}{\alpha} \\
    \Problim{i}{-\log{\max\limits_{x} \Prob{X_i = x \,|\, Y = y}} < \Delta -\log{\alpha}} &\leq \frac{1}{\alpha} \\
    \Problim{i}{\minent{X_i\, |\, Y} < \Delta -\log{\alpha}} &\leq \frac{1}{\alpha}
\end{align}
...
\begin{lemma}
Let $\mathcal{W}$ be a distribution family indexed by the random family $Z$ and suppose that no $(\zo^n, \mathcal{W}, \tilde{m}, t, \epsilon, \delta, 2\ell)$-secure sketch with distributional advice exists.  For all families $\mathcal{W}'\subseteq \mathcal{W}$ with probability mass functions $Z'\subseteq Z$ where $\Pr[Z\in Z']\ge 1-2^{-\zeta}$ 
...
\paragraph{Interpretation}
The natural parameter setting of the above is setting $\zeta = \max\{\tilde{m},1\} $ which shows that half of the distributions have no secure sketch extractor.  In the setting considered later in this work where $\tilde{m} = \Theta(n)$ this shows that almost all distributions have no secure sketch.
