...
\end{enumerate}
For a $(1/4-(\epsilon_1+\epsilon_2+\epsilon_3)/2)$-fraction of the distributions $W_z\in\pcodeu$ there is no $(\{0,1\}^n, W_z, \kappa, t, \epsilon, \ell)$-fuzzy extractor for 
$\epsilon<1/3 - 2(\epsilon_1+\epsilon_2+\epsilon_3)/3.$ 
For 
...
\item Lemma~\ref{lem:entr of members} for input $z \in \znk$ the advice string can only reduce the entropy of almost all viable points by a small amount. We call such points \textbf{Average Points}.  There are some points that the adversary has a large amount of information on that we call \textbf{Free Points}.

\item Corollary~\ref{corollary:info loss} puts together the above two Lemmas to show that the adversary includes a small number of points from the particular distribution $W_z$ in the viable set.
...
\item[Section~\ref{ssec:proof main theorem}] We present the proof of Theorem~\ref{thm:main theorem} based on the above lemmas and corollary, 
...
Lemma~\ref{lem:entr of members} bounds how much information $\advice$ contains about the points in the distribution.  Let $(\gen, \rep, \aux)$ be a fuzzy extractor with distributional advice. Let $\advice$ be some string.  For a tuple $(v, p, r, \advice)$ define 
\[
...
\begin{lemma}
Let $\mu$ be defined as in Lemma~\ref{lem:smallgeneralviable}.  For a particular $z$ let $w_{z_1},..., w_{z_{2^k}}$ represent the support of $W_z$ (ordered lexicographically). For every distribution $A$ over $\zo^\ell$ (the length of the advice string), there is a family of sets $W^*_a$  where $|W^*_a| \le \nu+1$ such that for all $w\in W_z\setminus W_a^*$ it is true 
\[
\log{\Pr_{z\leftarrow \znk}\left[\viable(w_{z_i}, p, \mathsf{key}, A) = 1 \right]}\le -\frac{\alpha -\ell }{2^k}+1+\mu+k-\log{\nu}.
\]
...
\begin{align*}
\expe_{Z} \left[\sum_{\mathsf{key} \in \mathtt{Goodkey}_p}\sum_{w_{z,i}\not\in\mathcal{I}_\advice} \viable(w_{z,i}, p, \mathsf{key}, \advice) \middle| \begin{aligned} z\leftarrow Z\\ \advice\leftarrow \aux(z)\\w\leftarrow W_Z\\(r, p)\leftarrow \gen(w, \advice)\end{aligned} \right]< 2^{-\frac{\alpha - |\advice|}{2^k}+1+\mu+2k-\log{\nu}}.
\end{align*}
And finally, 
\begin{align*}
\expe_{Z} \left[\sum_{\mathsf{key} \in \mathtt{Goodkey}_p}\sum_{w_{z,i}}  \viable(w_{z,i}, p, \mathsf{key}, \advice) \middle| \begin{aligned} z\leftarrow Z\\ \advice\leftarrow \aux(z)\\w\leftarrow W_Z\\(r, p)\leftarrow \gen(w, \advice)\end{aligned} \right]< 2^{-\frac{\alpha-|\advice|}{2^k}+1+\mu+2k-\log{\nu}}+\nu+1.
\end{align*}
...
Let $A$ be an random variable over $\zo^\ell$. Let $\znk$ as in Lemma~\ref{lem:entr of members}. We begin by recalling that $|\znkset| = 2^\alpha = {2^n\choose 2^k}$.  Thus,  
\begin{align*}
\Hoo(\znk)&=\alpha,\\
\Hav(\znk|A)&\ge \alpha - \ell.
\end{align*}
\noindent
%For a fixed value $z=z_1,..., z_{2^k}$ let $1_{z_i}$ denote the indicator random variable of whether $w_{z_i} \in \supp(W_z)$.
%\begin{claim}
For each value $w$, let $\alpha_{a,w}, \alpha_a$ denote the values 
\begin{align*}
2^{-\alpha_{a,w}}:&=\Pr_{z\leftarrow \znk | A=a}[w\in \supp(W_z)],\\
2^{-\alpha_a}:&=\Hav(\znk|A=a)=\max_{W\subseteq \zo^n | |W|=2^k}\left(\prod_{w\in W} 2^{-\alpha_{a,w}} \right) .
\end{align*}  
For a fixed value $a$ let $w_{1, a},...,w_{2^k, a}$ denote an arbitrary subset of $\zo^n$ of  size $2^k$. Then 
\[
...
\noindent
We need an elementary lemma which states that not too many of $\alpha_{a, w_i}$ are much larger than $\alpha_\alpha/2^k$:
\begin{claim}
    \label{lem:markovpred} Fix some value $a$.
    Define $W'_a \subset \{w_{1,a},..., w_{2^k, a}\}$ as \[W' =\left\{w \middle| \alpha_{a,w} \ge \frac{\alpha_{a}}{2^k} +(k-\log{\nu})\right\}.\] %as the set of all 
...
\[
    \Problim{i\leftarrow U_{\zo^k}}{\Pr_{z\leftarrow \znk | A=a}[w_{i,a}\in \supp(W_z)] \ge \beta \expe_i  \Pr_{z\leftarrow \znk | A=a}[w_{i,a}\in \supp(W_z)]}\le 1/\beta.
    \]
...
%Restating one has,  
%\begin{align*}
%\expe_{a\leftarrow A}\left(\max_{W\subseteq \zo^n | |W|=2^k}\left(\prod_{w\in W} 2^{-\alpha_{a,w}} \right)\right)\le 2^{-\alpha +\ell}.
%\end{align*}


 Note that \[\expe_{a\leftarrow A} \left(2^{-\alpha_a} \right) =\Hav(\znk|A)\ge 2^{-(\alpha-\ell)}.\] 
By Claim~\ref{lem:markovpred} it is true that there exists a set $W'_a\subseteq \zo^n$ of size at most  $\nu$ where  such that for all $w\not \in W'_a$ is true that $\alpha_{a, w}< \alpha_a/2^k+(k-\log{\nu}).$
%\[
...
Then 
\begin{align*}
-\log{\expe_{a\leftarrow A}\left[\Pr_{z\leftarrow \znk| A=a}[w\in \supp(W_z) | w\not \in W_a']\right]} &\ge -\log{\expe_{a\leftarrow A} \left(\max_{w\not\in W_a^*}\left(2^{-\alpha_{a,w}} \right) \right)} \\&\ge -\log{\expe_{a\leftarrow A}\left(   2^{-\alpha_a/2^k+k-\log{\nu}} \right)}\\&= \frac{\alpha-\ell}{2^k}-k+\log{\nu}.
\end{align*}
Let $w^*$ denote the point that will be given to $\gen$ that is $(\mathsf{key}, p)\leftarrow \gen(w^*, a)$.  We define the set $W^*_a = W'_a \cup \{w^*\}$.
Then,
\begin{align*}
-&\log{\expe_{a\leftarrow A}\left[\Pr_{z\leftarrow \znk| A=a \wedge \mathsf{key}\in \goodkey, p}[w\in \supp(W_z) | w\not \in W_a']\right]} \ge\\
-&\log{\expe_{a\leftarrow A}\left[\Pr_{z\leftarrow \znk| A=a \wedge w^*\in \supp(W_z)}[w\in \supp(W_z) | w\not \in W_a^*]\right]}\ge \frac{\alpha-\ell}{2^k}-k+\log{\nu}-1.
\end{align*}
...
We now proceed to bounding $\viable(w_i, p, \mathsf{key})$.  By assumption, By Lemma~\ref{lem:smallgeneralviable} we know that there are at most $2^\mu$ points in $\viable(w_i, p, \mathsf{key})$.  Thus, by union bound for any point $w\in \zo^n$,
\[
\log{\expe_{a\leftarrow A}\left[\Pr[\viable(w, p, \mathsf{key}, a)=1 \wedge w\in \supp(W_z) \middle| \begin{substack}{z\leftarrow \znk| A=a\\w^*\leftarrow W_z\\\mathsf{key}, p\leftarrow \gen(w^*, a)\\\mathsf{key}\in \goodkey, p\\w\not \in W_a^*}\end{substack}]\right]} \le -\frac{\alpha -\ell }{2^k}+1+\mu+k-\log{\nu}.
\]

%\begin{align*}
%\log{\Pr\left[\viable(w_{z,i}, p, \mathsf{key}, \advice) = 1 \right]}&\le - (\eta-1 -\mu) \\
%&=-\left(\frac{\alpha -\ell }{2^k}-1-\mu-\left(k-\log{\nu}\right)\right).
%\end{align*}
