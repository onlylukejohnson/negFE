...
\label{lem:convert distinguisher}
    ...
    There is no $(\mathsf{Gen, Rep})$ that is a $(\mathcal{M},\wnk,\kappa, t, \ell, \epsilon)$-fuzzy extractor with distributional advice for  if
\[
...
Lemma~\ref{lem:entr of members} bounds how much information $a$ contains about the points in the distribution.  Note that we consider a fixed $a$, considering the expectation across $a\in A$ after Corollary~\ref{corollary:info loss}.

\begin{lemma}
Let $\mu$ be defined as in Lemma~\ref{lem:smallgeneralviable}.  Define the notation
...
\end{proof}
\noindent
By Claim~\ref{lem:markovpred} it is true that there exists a set $\easy_a\subseteq \zo^n$ of size at most  $\nu$ where  such that for all $w\not \in \easy_a$ is true that $\alpha_{a, w}< \alpha_a/2^k+(k-\log{\nu}).$
...
\[
\hviable(w, p, \key, a, \easy_{a,w^*}) \le 2^{-\frac{\alpha -\ell }{2^k}+1+\mu+k-\log{\nu}}.
\]
\noindent
...
\end{align*}\todo{save this for later}
...
With Corollary~\ref{corollary:info loss} in hand we are ready to prove Lemma~\ref{lem:convert distinguisher} in earnest.
