...
\]
First note that \[\Pr_{(x, y, z) \leftarrow X', Y', Z' | X'\in\goodsketch_{Y', \aux(Z')}}[\mathcal{D}(x,y,z)=1]\ge 1-2\epsilon.\]
...
One then has, 
\begin{align*}
&2^{-\Hav(X'|Y', Z',X'\in\goodsketch_{Y', \aux(Z')})} \\&\ge \Pr_{x, y, z\leftarrow (X', Y', Z') | X'\in\goodsketch_{Y', \aux(Z')}} [\mathcal{D}(x, y, z)=1] 2^{-\Hav(X'|Y', Z', X'\in\goodsketch_{Y', \aux(Z')}, \mathcal{D}(X', Y', Z')=1) }\\&+ \Pr_{x, y, z\leftarrow (X', Y', Z') | X'\in\goodsketch_{Y', \aux(Z')}} [\mathcal{D}(x, y, z)=0] 2^{-\Hav(X'|Y', Z', X'\in\goodsketch_{Y', \aux(Z')}, \mathcal{D}(X', Y', Z')=0) }\\
...
\noindent\todo{stopped here}
We now proceed to the proof of Theorem~\ref{thm:main theorem ss}.   Let \[\chi:= -\log{1-2\epsilon_\sketch} +1 + 2\max\left\{-\frac{\alpha+-|\advice|}{2^k}+1+\mu+2k-\log{\nu}, \log{\nu+1}\right\}.\]  Restating Lemma~\ref{lem:convert distinguisher ss} one has that 
\[
