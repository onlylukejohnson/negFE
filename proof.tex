...
\item[Condition 3] That $k\ge \gamma + \log{n+ \omega(\log{n})}$ which implies $2^{k-\gamma}\ge n+\omega(\log{n})$. 
\end{description}
...
Below we consider two different parameter settings when one is trying to derive a nontrivial key $\kappa = \omega(\log n)$ and when one is deriving a maximal length key where $\kappa = \Theta(n)$. In both settings, we assume that
\begin{description}
\item[Condition 4] For some constant $0<c_{k,1} < 1$, we set $k = \gamma + c_{k,1}n$. 
\end{description}
\paragraph{Setting of $\kappa =\omega(\log n)$} 
\begin{description}
\item[Condition 5a]
Let $c_{k,2}$ be some constant such that $c_{k,2}<c_{k,1}$ and assume that 
\[
...
\[
\log{\epsilon_1} \le  -\left((1+c_{\kappa})\kappa+\frac{(n-3k)2^k-\ell}{2^k} - nh_2(1/2-t/n) \right) \le  -(1+c_{\kappa})\kappa = -\omega(\log{n}).
\]
\noindent
...
In this setting,
\begin{description}
\item[Condition 5b] Set 
\begin{align*}
\ell&\le  3(c_{k,1}-c_{k,2})n2^{\gamma+c_{k,1}n}
\end{align*}
\item[Condition 6b] Let $c_{\kappa^*} n = \kappa+\log{\nu}$ and 
\begin{align*}
...
\subsubsection{Proof of Theorem~\ref{thm:main theorem ss}}
