...
\begin{lemma}
$\mu$ be defined as in Lemma~\ref{lem:smallgeneralviable}.  
For $\znk$ define the random variable $\advice\leftarrow \aux(\znk)$.  For each value $a$ in the support of  there is some set $\mathcal{I}_{\advice}$ of size at most $\nu+1$, it is true for each $i\not \in \mathcal{I}_\advice$ that
\[
...
Let $\advice\leftarrow \aux(\znk)$ be defined as above. We begin by recalling that $|\znk| = 2^\alpha = {2^n\choose 2^k}$.  Thus,  
\begin{align*}
\Hoo(\wnkz)&=\alpha,\\
\Hav(\wnkz|\advice(\znk))&\ge \alpha - \ell.
\end{align*}
\noindent
For all values $x, y$, 
\[
\Pr_{z\leftarrow \znk}[x\in \supp(W_z) | y\in \supp(W_z)] = \begin{cases} \frac{1}{2^n-1} &x\neq y\\0&x=y.\end{cases}
\]
\begin{claim}
One has that 
\begin{align*}
...
\end{claim}
\begin{proof}
For a sequence of distinct  $w_1,..., w_i$ it holds that 
\[
\left[ \Pr[w_i\in \supp(W_Z) | \advice(Z), w_1,..., w_{i_1} \in \supp(W_Z)\right]\ge \left[ \Pr[w_i\in \supp(W_Z) | \advice(Z)\right].
\]
