...
\noindent
Now by Lemma~\ref{lem:markovpred} it is true that there exists a set $\mathcal{I}\subseteq \{1,...,2^k\}$ where $\beta: =  2^k/| \mathcal{I} | = 2^k/\nu$ such that for all $i\not \in \{1,...,2^k\} \setminus\mathcal{I}$ is true that 
\[
\Hav(W_{z,i} | \advise) \ge \frac{\alpha -|\advise| - \log \beta}{2^k} = \frac{\alpha +\log{\nu}-|\advise| - k}{k}.
\]
...
Let \[\gamma:=\frac{\alpha -|\advise| - \log \beta}{2^k},\] 
...
\[
\epsilon<1/2-\left(2^{-\frac{\alpha+\log{\nu}-|\advise|-k}{2^k}+1+\mu+k}+\frac{\nu+1}{2^{\kappa}}\right).
\]
