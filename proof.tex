...
\[
\Pr\left[\viable(w_{z,i}, p, \mathsf{key}) = 1\middle| \begin{aligned} z\leftarrow Z\\ \advise\leftarrow \aux(z)\\w\leftarrow W_Z\\(\mathsf{key}, p)\leftarrow \gen(w, \advise) \\\mathsf{key}\in\mathtt{GoodKey}\end{aligned} \right]< \frac{2^{\frac{|\advise|+\log{k}-\log{\ell}}{k}+1+\mu}}{{2^n\choose k}}.
\]
\end{lemma}
\begin{proof}
Let $Z$ and $\advise$ be defined as above. We begin by noting that \[
\Hoo(W_{z, 1},..., W_{z, k}) = \log {2^n\choose k}\overset{def}=\alpha.\]
...
\end{align*}
\end{proof}
By Lemma~\ref{lem:info loss} the expected number of points outside of $\mathcal{I}_\advise$ that are included in $\viable$ is at most 
\begin{align*}
\expe_{Z} \left[\sum_{i} \viable(w_{z,i}, p, \mathsf{key}, \advise) \middle| \begin{aligned} z\leftarrow Z\\ \advise\leftarrow \aux(z)\\w\leftarrow W_Z\\(\mathsf{key}, p)\leftarrow \gen(w, \advise) \\\mathsf{key}\in\mathtt{GoodKey}\\w_{z,i}\not\in\mathcal{I}_\advise\end{aligned} \right]< \frac{2^{\frac{|\advise|+\log{k}-\log{|\mathcal{I}|}}{k}+1+\mu+k}}{{2^n\choose k}}.
\end{align*}
\todo{do we need to sum over the keys?}

\begin{lemma}
\label{lem:convert distinguisher}
Let all parameters be as in Lemma~\ref{lem:info loss}.  Then there is no $(\{0,1\}^n, \mathcal{W}, \kappa, t, \epsilon, \ell)$-fuzzy extractor with distributional advise (see Definition~\ref{def:fe distributional}) under the following conditions:
\begin{enumerate}
\item $\kappa\ge $
\item $\epsilon <$.
\end{enumerate}
...
\item Else output $1$.
\end{enumerate}
First note that by perfect correctness it is always the case that when given $\mathsf{key}, p$ that $\mathcal{D}$ outputs $1$.  We proceed to bound the probability that $\mathcal{D}$ outputs $1$ when given $U_\kappa, p$.  We bound the number of parts with at least one point in viable.  We begin by assuming that all points in viable are in different  values $r$ so the bound on the size of $|\viable  |+ \ell$ gives a bound on the number of nonempty parts.
