...
\end{enumerate}
For $2^{-\tilde{m}}$ of the distributions in the family $\pcodeu$ (denoted as $z\in\pcodeset$) there is no $(\{0,1\}^n, W_z, \tilde{m}, t, \epsilon,\delta, \ell)$-secure sketch for 
\[
...
Let all parameters be as in Corollary~\ref{corollary:info loss ss} with $\nu \in\mathbb{Z}^+$.  Then there is no $(\{0,1\}^n, \mathcal{W}, \tilde{m}, t, \epsilon_\sketch, \ell, \delta)$-secure sketch with distributional advice if
\[
\tilde{m}> -\log{1-\epsilon_\sketch} +1 + 2\max\left\{-\frac{\alpha-|\advice|}{2^k}+1+\mu+2k-\log{\nu}, \log{\nu+1}\right\}.
\]
\noindent
Furthermore, there exists an algorithm $\mathcal{D}$ that always outputs $1$ when given samples of the form $w, ss, z$ that are correctly generated by the secure sketch. (This fact will be used to bound the statistical distance between triplets $w, ss, z$ and distributions with average min-entropy conditioned on $ss, z.$)
\end{lemma}
...
\todo{stopped here}
\begin{align*}
\Delta((X, Y, Z, X\overset{?}\in \goodsketch_{Y, \aux(Z)}), (X', Y', Z', X'\overset{?}\in\goodsketch_{Y', \aux(Z')})) &\le \epsilon.
\end{align*} Our goal is to bound the min-entropy of $X', Y', Z' | X'\in\goodsketch_{Y', \aux(Z')}$. Noting that by Claim~\ref{clm:smooth ent cond} the min-entropy without this second condition increases by at most $1$. % | X' \in \goodsketch_{Y', \advice(Z')}$.  Note that \[\Pr_{(x,y, z)\leftarrow (X', Y', Z')| \mathcal{I}'=1}[\mathcal{D}(x,y,z)=1]\ge 1-2\epsilon.\] Note that 
\[
\Pr[\viable(W, \sketch(W), Z) = 1| \sketch_W \in \goodsketch_W] \ge 1/2.
\]
Let $A':= \aux(Z')$.  Note that for By the definition of $\viable$, the support of $X'$ must be drawn from points in $W_Z'$.  For any fixed value of $y\in Y$ by Corollary~\ref{corollary:info loss ss}, it is true that 
\begin{align*}
...
\]
Lastly, define $\mathcal{D}(v, ss, z) =1$ if $ss\not\in\goodsketch_v$ or $\viable(v, ss, \aux(z), z) =1$.  $\mathcal{D}$ outputs $0$ otherwise.  Note that for $w, ss, z$ correctly generated as the output of $\sketch$ $\mathcal{D}$ always outputs $1$. 
