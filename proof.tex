...
\[
\epsilon<   \frac{1-\left(\frac{2^{\frac{|\ell|+\log{k}-\log{\nu}}{k}+1+\mu+k}}{{2^{\alpha/k+\kappa}}}+\frac{\nu+1}{2^\kappa}\right)}{2}.
\]
\end{lemma}
...
\todo{Make it clear on the connection with with Lemma~\ref{lem:close family}.  In particular, I think we need twice that value for statistical distance}
\[
