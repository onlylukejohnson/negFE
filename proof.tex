...
Fuller, Reyzin, and Smith~\cite[Lemma 7.3]{fuller2020fuzzy} showed that good secure sketches are bounded in size as they imply good Shannon error correcting codes.  This result holds true if one considers a secure sketch that retains smooth min-entropy with no loss in parameters.  

\begin{lemma}
\label{lem:smallgeneralviable ss}
    Suppose $\mathcal{M}$ is $\zeroone{n}$ with the Hamming Metric. Suppose $(\sketch, \rec)$ is a $(\zo^n, \mathcal{W}, \tilde{m}, t, \epsilon, \delta)$-secure sketch, for some family $\mathcal{W}$. 
...
\[
\Haveps(W| \sketch(W)) \le \Haveps(W| \sketch(W), \goodsketch_W) +1.
\]
Let $X', Y'$ be distributions and $\eta'$ be an event where $\Delta((X, Y, \eta), (X', Y', \eta'))\le \epsilon$. Consider the following distinguisher $\mathcal{D}$ for tuples of the form $a, b, c, d$:
\begin{enumerate}
\item If $c\neq 1$ output $0$,
\item If $\viable(a, b, \advise(d)) =0 $ output $0$,
\item If $\Pr[W_z = w] = 0$ output $0$.
\item Output $1$.
\end{enumerate}
Note that when given $(w, ss, \goodsketch_w, z)|\goodsketch_w$ that $\mathcal{D}$ outputs $1$ with probability $1$.  
Thus, it must be the case that 
\[
\Pr[D(x', y', \eta') = 1] \ge 1- \epsilon.
\]
That is, 
$\Pr[x'\in W_z] \ge 1-\epsilon.$
Recall that by Corollary~\ref{corollary:info loss ss} there are most 
