...
the family $\mathcal{W'}$.  We use the distribution $z\leftarrow Z$ indicate this choice, and use the notation $W_Z$ to indicate sampling $W$ uniformly from $\mathcal{W}$ with $Z$ describing the outcome of this sampling. For all $z$, the goal is to build a good fuzzy extractor for $W_z$.  The impossibility result shows this is impossible for an average $z$.  Importantly, in the
impossibility result the adversary knows the outcome $Z=z$ but not the
individual point $w\leftarrow W_z$ that was input to $\gen$.
